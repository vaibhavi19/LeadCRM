...
class conn {
    ...
    function getcon() {
        $this->link = mysqli_connect(DB_SERVER_IP, DB_LOGIN, DB_PASSWORD, DB_DATABASE);
...
//echo $query;exit;
 ...
 function save_bill_log_for_mail($log_description, $bill_id,$created_by) {
        $query = $error_message = "";
       // $created_by = (isset($_SESSION['user_id']) ? $_SESSION['user_id'] : "");
        $ip_address = (isset($_SESSION['ip_address']) ? $_SESSION['ip_address'] : $_SERVER['REMOTE_ADDR']);
...
function mysqldateformatddmmyyyy($str) {
    $str = str_replace(" ", "", $str);
    if ($str == "") {
        return "";
    } else {
...
function message_show($message, $message_type) {
    $bg_color = "#e9ffd9";
    $border_color = "#a6ca8a";
...
function show_validation_message($object_id, $object_message) {
...
function replaceDateTime($dateval) {
    if ($dateval == "")
        $dateval = "null";
    else {
...
function db_date_to_print($dateval) {
    if ($dateval == "")
        return "";
    return date('d-m-Y', strtotime($dateval));
}

function encryptdecrypt($data, $encrypt) {
    if ($encrypt == "true")
        $intValue = 1;
    else
        $intValue = -1;
...
function pivot_columns($table_name, $static_columns, $ctab_text_column, $ctab_value_column, $where_condition, $group_by, $order_by, $no_total) {
    $obj = new conn();
...
function validate_invoice_date($invoice_date, $department) {
...
function validate_invoice_due_date($invoice_date, $invoice_due_date) {
    if ($invoice_due_date == '') {
