                    ...
                    <div class="card card-orange ">
                        ...
                        if (isset($_REQUEST['submit'])) {
                            if ($_REQUEST['action'] == 'NEW') {
                                $query = "insert into `gm_role_master`(`role_desc`,default_page,`status`,`created_on`,`created_by`,`client_id`)
                values('$roledesc','$default_page','A',now(),'{$_SESSION['user_id']}'," . replaceBlank($_SESSION['client_id']) . ")";
                            } elseif ($_REQUEST['action'] == 'EDIT') {
                                $query = "update `gm_role_master` set `role_desc` = '$roledesc', default_page='$default_page',`status` = '$status', `edited_on` = now(), `edited_by` = '{$_SESSION['user_id']}',client_id=" . replaceBlank($_SESSION['client_id']) . " where `role_id` = '$pkey_id'";
                            }
                            //echo $query;exit;
                        ...
                        <form role="form" id="roleForm" name="departmentForm" method="post" enctype="multipart/form-data">
                            ...
                            <div class="card-footer">
                                ...
                                <a href="../forms/listview.php?id=3" class="btn btn-danger">Back</a>
                            </div>
                        </form>
                    </div>
                </div>
